...
class SemEvalTask4BProcessor(object):
  """Base class for data converters for sequence classification data sets."""
  def __init__(self):
    ...
    df_input["label"] = df_ans["ans"]
    df_input.to_csv("SemEval20_task4B_input.csv")
  ...
  def _create_examples(self, lines, set_type):
    ...
    for line in lines.itertuples():
      guid = line.id
      text_a = line.FalseSent
      text_b = line.OptionA
      label = 1 if line.label == 'A' else 0
      ...
      text_a = line.FalseSent
      text_b = line.OptionB
      label = 1 if line.label == 'B' else 0
      print(guid, text_a, text_b, label)
      examples.append(InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label))
      ...
      text_a = line.FalseSent
      text_b = line.OptionC
      label = 1 if line.label == 'C' else 0
      print(guid, text_a, text_b, label)
      examples.append(InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label))
...
def main(_):
  ...
  processors = {
      ...
      'sts-b': StsbProcessor,
      'imdb': ImdbProcessor,
      "yelp5": Yelp5Processor, "semevaltask4b": SemEvalTask4BProcessor
  }
